...
namespace SyncFoldersTests
{
    ...
    /// </summary>
    //*******************************************************************************************************
    public class T100_UncommonSituations
    {
        ...
        //===================================================================================================
        /// <summary>
        /// This test tests most common situations with a new file with errors in directory 2 plus a chk file
        /// with errors, however the file should be recovered from the chk file 
        /// </summary>
        //===================================================================================================
        [Test]
        public void Test02_NewFileTwoFolders_FileIn2_PlusChk_Recover()
        {
            Random oRandom = new Random((int)DateTime.UtcNow.Ticks);
            foreach (int nFileSizeKB in new int[] {
                16, 17,
                ...
                200 * 1024, 200 * 1024 + 1})
            {
                for (int nConfiguration = 0; nConfiguration < 1024; ++nConfiguration)
                {
                    ...
                    oFs.CreateTestFile(strPath2, 2, nFileSizeKB * 1024, dtmTimeToUse, true, false,
                        null, null, null, true);

                    List<long> oErrorsInOriginalFile = new List<long>();
                    oErrorsInOriginalFile.Add(0);
                    oFs.SetSimulatedReadError(strPath2, new List<long>(oErrorsInOriginalFile));

                    List<long> oErrorsInOriginalSavedInfo = new List<long>();
                    oErrorsInOriginalSavedInfo.Add(4096);
                    oFs.SetSimulatedReadError(strPathSavedInfo2, new List<long>(oErrorsInOriginalSavedInfo));
                    ...
                    if (oSettings.FirstToSecond && oSettings.FirstToSecondDeleteInSecond)
                    {
                        Assert.IsTrue(!oFs.Exists(strPath2));
                    }
                    else if (oSettings.FirstToSecond)
                    {
                        Assert.IsTrue(!oFs.Exists(strPath1));

                        if (oSettings.TestFiles && oSettings.RepairFiles)
                        {
                            Assert.IsTrue(oFs.IsTestFile(
                                strPath2, 2, nFileSizeKB * 1024, dtmTimeToUse,
                                true,
                        ...
                        else
                        {
                            Assert.IsTrue(oFs.IsTestFile(
                                ...
                                true,
                                false, null,
                                new List<long>(oErrorsInOriginalFile),
                                new List<long>(oErrorsInOriginalSavedInfo)
                                ));
                        }
                    }
                    else
                    {
                        if (!oSettings.TestFiles || !oSettings.RepairFiles)
                        {
                            ...
                            Assert.IsTrue(oFs.IsTestFile(
                                strPath1, 2, nFileSizeKB * 1024, dtmTimeToUse, oSettings.CreateInfo, false,
                                null, null, null
                                ));

                            Assert.IsTrue(oFs.IsTestFile(
                                strPath2, 2, nFileSizeKB * 1024, dtmTimeToUse, true, false,
                                null, new List<long>(oErrorsInOriginalFile),
                                oSettings.CreateInfo && (!oSettings.FirstToSecond || !oSettings.FirstReadOnly) ? null : new List<long>(oErrorsInOriginalSavedInfo)
                                ));
                            ...
                            if (nFileSizeKB > 0)
                            {
                                ...
                                Assert.IsTrue(oFs.IsTestFile(strPath1, 2, nFileSizeKB * 1024, dtmTimeToUse,
                                    oSettings.CreateInfo,
                                    false, null, new List<long>(oErrors), null
                                    ));
                        ...
                        else
                        {
                            ...
                            if (nFileSizeKB > 0)
                            {
                                ...
                                if (oSettings.TestFiles && oSettings.RepairFiles)
                                {
                                    // there it is expected that the second file should remained the same
                                    // and the first file shold have been repaired
                                ...
                                else
                                {
                                    ...
                                    Assert.IsTrue(oFs.IsTestFile(
                                        strPath1, 2, nFileSizeKB * 1024, dtmTimeToUse, true, false,
                                        null, new List<long>(oErrors), null
                                        ));
